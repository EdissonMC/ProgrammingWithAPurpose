...
public class EqualStacks2 {
    ...
    public static int checkStacks(List<Integer> h1, List<Integer> h2, List<Integer> h3) {
        ...
        int sumaStkRef = sumStack(arr.get(0));
        ...
        while (pointer < arr.size()) {
            ...
            // sumIter = sumStack(arr.get(pointer));

            while (sumIter > sumaStkRef) {
                sumIter = sumIter - arr.get(pointer).pop();
            }

            if (sumIter == sumaStkRef) {
                pointer++;
                sumIter = sumaStk3;
            }
            else {
                sumaStkRef = sumaStkRef - arr.get(0).pop();
            }

        }


        System.out.println(sumaStkRef);
